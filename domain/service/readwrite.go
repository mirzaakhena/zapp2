...
func basic(pkg *entity.ThePackage, templateFile []byte, outputFilePath string, object interface{}, perm os.FileMode) {
  ...
  t := template.Must(template.New("codes").Funcs(funcMap).Parse(string(templateFile)))
...
// HasTime is
func HasTime(dataTypes []entity.TheField) bool {
...
// GetUniqueFields is
func GetUniqueFields(dataTypes []entity.TheField, currentEntityName string) []entity.TheField {
  uniqueFields := make([]entity.TheField, 0)
  existing := map[string]entity.TheField{}
  for _, tm := range dataTypes {
